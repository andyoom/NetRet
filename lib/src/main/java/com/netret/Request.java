...
public class Request implements Runnable {
    ...
    public Request(final URLData data, final List<RequestParameter> params,
                ...
                if (statusCode == HttpStatus.SC_OK) {
                    ...
                    if ((response.getEntity().getContentEncoding() != null)
                        ...
                        if (response.getEntity().getContentEncoding()
                            ...
                            strResponse = Request.inputStreamToString(is);
                            is.close();
                        } else {
                    ...
                    if (responseInJson.hasError()) {
                        if (responseInJson.getErrorType() == 1) {
                            handler.post(new Runnable() {
                                @Override
                                public void run() {
                                    callback.onCookieExpired();
                                }
                            });
                        } else {
                            handleNetworkError(responseInJson.getMsg());
                        }
                    } else {
                        // 把成功获取到的数据记录到缓存
                        if (urlData.getNetType().equals(REQUEST_GET)
                            ...
                            CacheManager.getInstance().putFileCache(newUrl,
                                    responseInJson.getResult().toString(),
                                    urlData.getExpires());
                        }
                        ...
                        handler.post(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                callback.onSuccess(responseInJson
                                        .getResult());
                            }
    ...
    void setHttpHeaders(final HttpUriRequest httpMessage) {
        headers.clear();
        headers.put(Constants.ACCEPT_CHARSET, "UTF-8,*");
        headers.put(Constants.USER_AGENT, "Young Heart Android App ");
        headers.put(Constants.ACCEPT_ENCODING, "gzip");
